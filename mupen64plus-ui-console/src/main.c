...
static m64p_handle l_ConfigAudio = NULL;
...
static m64p_error OpenConfigurationHandles(void)
{
    ...
    rval = (*ConfigOpenSection)("Audio-SDL", &l_ConfigAudio);
    if (rval != M64ERR_SUCCESS)
    {
        DebugMessage(M64MSG_ERROR, "failed to open 'Audio-SDL' configuration section");
        return rval;
    }
...
static m64p_error ParseCommandLineMain(int argc, const char **argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
		...
		{
			(*ConfigSetParameter)(l_ConfigCore, "SaveStatePath", M64TYPE_STRING, argv[i + 1]);
...
#undef PARSE_GB_CART_PARAM
		...
		else if (strcmp(argv[i], "--online") == 0)
		{
			...
			if (strcmp(argv[i+1], "1") == 0) {
				...
				DebugMessage(M64MSG_ERROR,"Player ID Invalid, must be between 1 and 4. Currently is: %s", argv[i + 1]);
			}
			i += 1;
		}
        else if (strcmp(argv[i], "--vol") == 0)
        {
            int defVol = atoi(argv[i + 1]);
            (*ConfigSetParameter)(l_ConfigAudio, "VOLUME_DEFAULT", M64TYPE_INT, &defVol);
            i += 1;
        }
...
int main(int argc, char *argv[])
{
	...
	if (Online == 1)
	{
		...
		if ((*CoreDoCommand)(M64CMD_NETPLAY_GET_VERSION, 0x010000, &version) == M64ERR_SUCCESS)
		{
			...
			if ((*CoreDoCommand)(M64CMD_NETPLAY_INIT, 45000, "127.0.0.1") == M64ERR_SUCCESS) {
				...
				DebugMessage(M64MSG_INFO, "Netplay: registering player: %d | id: %d", Player, reg_id);
				if ((*CoreDoCommand)(M64CMD_NETPLAY_CONTROL_PLAYER, Player, &reg_id) == M64ERR_SUCCESS) {
					DebugMessage(M64MSG_INFO, "Netplay: registered for player %d", Player);
				}
				else {
					DebugMessage(M64MSG_INFO, "Netplay: registered for player %d FAILED", Player);
				}

				DebugMessage(M64MSG_INFO, "\n->Netplay: Started Online as Player %d", Player);
			}
			else{DebugMessage(M64MSG_INFO, "Netplay: init FAILED");}
		}
	}
	else
	{
		DebugMessage(M64MSG_INFO, "\n->Netplay: Started Offline");
	}
